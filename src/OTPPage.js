import { useState } from 'react';
import axios from 'axios';

function OTPPage() {
  const [email, setEmail] = useState('');
  const [otp, setOtp] = useState('');
  const [otpSent, setOtpSent] = useState(false);
  const [loading, setLoading] = useState(false);

  const fileId = localStorage.getItem('fileId');

  const sendOTP = async () => {
    try {
      setLoading(true);
      await axios.post(`http://localhost:5000/api/files/${fileId}/otp`, {
        email: email
      });
      setOtpSent(true);
      alert('OTP sent to your email!');
    } catch (error) {
      alert('Failed to send OTP! Make sure backend is running.');
    } finally {
      setLoading(false);
    }
  };

  const verifyOTP = async () => {
    try {
      setLoading(true);
      const response = await axios.post(
        `http://localhost:5000/api/files/${fileId}/verify`, {
        email: email,
        otp: otp
      });
      localStorage.setItem('token', response.data.token);
      localStorage.setItem('receiverEmail', email);
      alert('OTP verified! Opening file...');
      window.location.href = '/file';
    } catch (error) {
      alert('Wrong OTP! Try again.');
    } finally {
      setLoading(false);
    }
  };

  return (
    <div style={{ minHeight: "100vh", backgroundColor: "#0d0d1a",
    fontFamily: "Arial", display: "flex", flexDirection: "column",
    alignItems: "center", justifyContent: "center" }}>

      <div style={{ backgroundColor: "#1a1a2e", borderRadius: "16px",
      padding: "40px", width: "100%", maxWidth: "500px",
      boxShadow: "0 0 30px rgba(0,212,255,0.1)" }}>

        <h1 style={{ color: "#00d4ff", textAlign: "center" }}>
          🔐 Verify Your Identity
        </h1>
        <p style={{ color: "#888", textAlign: "center" }}>
          Enter your email to receive OTP
        </p>

        <input
          type="email"
          placeholder="Enter your email"
          value={email}
          onChange={(e) => setEmail(e.target.value)}
          style={{ width: "100%", padding: "12px", marginTop: "20px",
          borderRadius: "8px", border: "1px solid #333",
          backgroundColor: "#0d0d1a", color: "#fff",
          boxSizing: "border-box" }}
        />

        <button
          onClick={sendOTP}
          style={{ marginTop: "16px", width: "100%", padding: "14px",
          background: "linear-gradient(90deg, #00d4ff, #0077ff)",
          color: "white", border: "none", borderRadius: "8px",
          fontSize: "16px", cursor: "pointer", fontWeight: "bold" }}>
          {loading ? '⏳ SENDING...' : '📨 SEND OTP'}
        </button>

        {otpSent && (
          <>
            <input
              type="text"
              placeholder="Enter OTP"
              value={otp}
              onChange={(e) => setOtp(e.target.value)}
              style={{ width: "100%", padding: "12px", marginTop: "20px",
              borderRadius: "8px", border: "1px solid #333",
              backgroundColor: "#0d0d1a", color: "#fff",
              boxSizing: "border-box" }}
            />

            <button
              onClick={verifyOTP}
              style={{ marginTop: "16px", width: "100%", padding: "14px",
              background: "linear-gradient(90deg, #00d4ff, #0077ff)",
              color: "white", border: "none", borderRadius: "8px",
              fontSize: "16px", cursor: "pointer", fontWeight: "bold" }}>
              {loading ? '⏳ VERIFYING...' : '✅ VERIFY & ACCESS FILE'}
            </button>
          </>
        )}

      </div>
    </div>
  );
}

export default OTPPage;
